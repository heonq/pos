...
- [ ] 상품 데이터를 서버에서 불러온다.
- [ ] 상품 데이터에 저장되어 있는 모든 상품에 대해 표시한다.
...
- [ ] 확인 버튼을 누르면 변경사항을 저장한다.
- [ ] 취소 버튼을 누르면 변경사항을 초기화하고 모달을 닫는다.
...
- 확인 버튼을 누르면 입력한 데이터에 대한 유효성 검사를 실시한다.
  ...
  - [ ] 0이 아니고 이미 존재하는 바코드일 경우 예외처리
  - [ ] 반드시 입력해야하는 값 : 이름,가격

- [ ] 카테고리는 선택하지 않을 경우 'none'으로 처리
- [ ] 바코드는 입력하지 않을 경우 0으로 처리

- [ ] 입력한 상품정보를 업데이트 후 서버에 저장한다.
- [ ] 취소 버튼을 누르면 변경내용을 취소하고 모달창을 닫는다.
...
- [ ] 판매내역을 서버에서 불러온다.
- [ ] 판매내역을 표시한다.
- [ ] 내역 수정 버튼을 누르면 해당 내역의 판매번호를 제외한 모든 영역이 input으로 변경된다.
- [ ] 수정 버튼의 텍스트가 저장 버튼으로 변경된다.
